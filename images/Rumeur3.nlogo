...
turtles-own
[
  know?           ;; if true, the turtle knows the rumor
  xmemory
  ymemory
  number-spread-by-word-of-mouth 
  number-spread-by-network
]
patches-own [meeting-point]
...
to setup
  ...
  ask n-of initial-persons-knowing-rumor turtles
    [ become-know ]
...
to setup-persons
  ...
  [
    ; for visual reasons, we don't put any persons *too* close to the edges
    setxy (random-xcor * 1) (random-ycor * 1)
    become-ignorant
  ]
end

to setup-patches
  ask patches [if random-float 1000 < Meeting-points-density [recolor]]
end
  

to setup-spatially-clustered-network
  let num-links (average-network-connections * number-of-persons) / 2
  ...
  repeat 10
  [
    layout-spring turtles links 0.3 (world-width / (sqrt number-of-persons)) 1
  ]
end
...
to move
  ...
  rt random 45
  lt random 45
  let meeting-point-i-see count  patches in-radius 2 with [meeting-point = true]
  ifelse meeting-point-i-see >= 1
  [fd 0.01]
  [fd 0.5]
  ]
end

to recolor  ; patch procedures "meeting-points"
  set pcolor grey
  set meeting-point true
end
  

to become-know  ;; turtle procedure
  set know? true
  set color blue
  set size 2
end
...
to spread-rumor
  ask turtles with [know?]
    [
      ask turtles in-radius 1 with [not know?]
  [if random 100 < Spread-by-word-of-mouth 
    [become-know
      set number-spread-by-word-of-mouth number-spread-by-word-of-mouth + 1]]
       ask link-neighbors with [not know?]
        [ if random-float 100 < Spread-by-network
            [ become-know
              set number-spread-by-network number-spread-by-network + 1 ] ] ] 
...
to check-border
  ...
  ask turtles with [know?] [
    if ((pxcor = -20) or (pxcor = 20) or (pycor = -20) or (pycor = 20) )[
      if ((pxcor = (- xmemory)) or (pycor = (- ymemory)) )  [
          become-ignorant
          ]
      ]
      set xmemory pxcor 
...
;  ask turtles with [know?]
;  [
;    if random 100 < percent-of-forgetfullness
;        [ become-ignorant ]   
;  ]
;end
@#$#@#$#@
...
"" ""
PENS
"know" 1.0 1 -13345367 true "" "plot (count turtles with [know?]) / (count turtles) * 100"
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="false">
    ...
    <metric>count turtles with [know?]</metric>
    <metric>count spread-by-network</metric>
    <metric>count spread-by-word-of-mouth</metric>
    <steppedValueSet variable="number-of-persons" first="10" step="10" last="100"/>
    <steppedValueSet variable="average-network-connections" first="0" step="1" last="8"/>
    <steppedValueSet variable="Meeting-points-density" first="0" step="1" last="10"/>
  </experiment>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="number-of-persons" first="10" step="10" last="100"/>
    <steppedValueSet variable="Meeting-points-density" first="0" step="1" last="10"/>
    <steppedValueSet variable="average-network-connections" first="1" step="1" last="8"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
